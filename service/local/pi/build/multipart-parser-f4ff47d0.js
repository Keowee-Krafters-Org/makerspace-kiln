import "node:fs";
import "node:path";
import { F as k, a as U } from "./index-7839c0d6.js";
import "express";
import "path";
import "url";
import "node:http";
...
--` + i;
  ...
  write(i) {
    let n = 0;
    const r = i.length;
    let E = this.index, { lookbehind: l, boundary: d, boundaryChars: p, index: e, state: a, flags: A } = this;
    const m = this.boundary.length, S = m - 1, N = i.length;
    let o, b;
    const h = (c) => {
      this[c + "Mark"] = n;
    }, s = (c) => {
...
function I(u) {
  const i = u.match(/\bfilename=("(.*?)"|([^()<>@,;:\\"/[\]?={}\s\t]+))($|;\s)/i);
  if (!i)
    return;
  const n = i[2] || i[3] || "";
...
async function se(u, i) {
  ...
  h.decode(), r.onPartBegin = function() {
    r.onPartData = S, r.onPartEnd = b, E = "", l = "", d = "", p = "", e = "", a = null, A.length = 0;
    ...
    if (l += h.decode(), E = E.toLowerCase(), E === "content-disposition") {
      ...
      s && (p = s[2] || s[3] || ""), a = I(l), a && (r.onPartData = N, r.onPartEnd = o);
    } else
      E === "content-type" && (e = l);
    l = "", E = "";
  };
